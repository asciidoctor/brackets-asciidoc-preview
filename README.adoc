...
also want to check btn:[Only sync on save] (see below). 
...
you may want to check btn:[Only sync on save] from the settings panel. This causes an update of the 
...
IMPORTANT: If the version of your Asciidoctor JavaScript extensions does not match the version of Asciidoctor used by this plugin, things might not work as expected. The current version is *Asciidoctor 1.5.6*.
